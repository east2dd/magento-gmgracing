...
class Magento_Template_Helper{
	...
	public function product_thumbnail_url(){
		if($this->inside_product_loop() && isset($this->magento_products[$this->i]['images']) && !empty($this->magento_products[$this->i]['images'])){
			return $this->magento_products[$this->i]['images'][0]['url'];
			// $_product = $this->magento_products[$this->i];
			// for($i=0;$i<count($_product['images']);$i++){
			// 	if (in_array($_product['images'][$i]['types'], 'thumbnail')){
			// 		return $_product['images'][$i]['url'];
			// 	}
			// }
			// return $_product['images'][0]['url'];
		}
	}
...
function magento_product_count(){
	global $Magento;
	return $Magento->product_count();
}
